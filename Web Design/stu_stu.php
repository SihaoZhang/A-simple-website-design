...
$nowid = $_SESSION['sid'];
 for ($i = 1; $i <= $_SESSION['size3']; $i++) {
 	...
 	if(isset($_POST[$index])){
 		$stuid = $_SESSION['fidm'.$i];
 		$open = $_SESSION['open'.$i];
 		break;
 	}
}
$sql = "SELECT * FROM student WHERE sid = '$stuid'";
$result = sqlresult($sql)or die ('some');
$row = mysqli_fetch_array($result);
$i = 1;
$query1="select * from student where sid='$nowid'";
$result1=sqlresult($query1) or die ('some');
$row1=mysqli_fetch_array($result1);
$name=$row1['sname'];
$presql = "SELECT * FROM friend WHERE sid = '$stuid' and fid = '$nowid'";
$preresult=sqlresult($query1) or die ('some');
$size4 = mysqli_num_rows($preresult);
$_SESSION['search3'] = mysqli_num_rows($result);
$_SESSION['fid'.$i] = $stuid;
      ...
      <nav class="navbar navbar-inverse">
		...
		<div id="page-wrapper">
			<div class="main-page general">
				<h2 class="title1">Personal Profile</h2>
				<div class="panel-info widget-shadow">
					<?php
						if($open == 1){
					?>
					<div class="col-md-6 panel-grids">
						<div class="panel panel-default"> <div class="panel-heading"> <h3 class="panel-title">Name</h3> </div> <div class="panel-body"><?php echo $row['sname'];?></div> </div>
					</div>
					<div class="col-md-6 panel-grids">
						<div class="panel panel-primary"> <div class="panel-heading"> <h3 class="panel-title">Email</h3> </div> <div class="panel-body"><?php echo $row['sid'];?></div> </div>
					</div>
					<div class="col-md-6 panel-grids">
						<div class="panel panel-success"> <div class="panel-heading"> <h3 class="panel-title">University</h3> </div> <div class="panel-body"><?php echo $row['university'];?></div> </div>
					</div>
					<div class="col-md-6 panel-grids">
						<div class="panel panel-info"> <div class="panel-heading"> <h3 class="panel-title">Major</h3> </div> <div class="panel-body"> <?php echo $row['major'];?></div> </div>
					</div>
					<div class="col-md-6 panel-grids">
						<div class="panel panel-warning"> <div class="panel-heading"> <h3 class="panel-title">GPA</h3> </div> <div class="panel-body"><?php echo $row['GPA'];?></div> </div>
					</div>
					<div class="col-md-6 panel-grids">
						<div class="panel panel-danger"> <div class="panel-heading"> <h3 class="panel-title">Information</h3> </div> <div class="panel-body"><?php echo $row['sinfo'];?></div> </div>
					</div>
					<div class="clearfix"> </div>
				</div>

				<div class="sub_home">
					<form form action="search_invite.php" method="POST">
					<?php 
					if ($size4==0){
					?>
						<input type="submit" name="invite'.$i.'" value="friend invitation"/>
					<?php
					}
					else{
					?>
						<input type="submit" name="delete'.$i.'" value="delete friend"/>
					<?php
					}
					?>
					</form>	
					...
					<div class="col-md-6 panel-grids">
						<div class="panel panel-primary"> <div class="panel-heading"> <h3 class="panel-title">University</h3> </div> <div class="panel-body"><?php echo $row['university'];?></div> </div>
					</div>
					<div class="clearfix"> </div>
				</div>
