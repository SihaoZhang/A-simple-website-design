...
<body class="cbp-spmenu-push">
	...
	<div class="cbp-spmenu cbp-spmenu-vertical cbp-spmenu-left" id="cbp-spmenu-s1">
      ...
      <nav class="navbar navbar-inverse">
		...
		<!-- main content start-->
		<?php			
			$sql1 = "SELECT * FROM jobpush AS jp, job AS j, company AS c WHERE jp.sid = '$nowid' and j.jid = jp.jid and j.cid = c.cid and jp.know = '1'";
			$sql2 = "SELECT * FROM share AS s, job AS j, company AS c, student as st WHERE s.sid = '$nowid' and j.jid = s.jid and j.cid = c.cid and s.fid = st.sid and s.know = '1'";
			$sql3 = "SELECT * FROM invitation AS i, student AS s WHERE i.iid = s.sid AND i.sid = '$nowid' AND i.status = 'Holding'";
		...
		<div id="page-wrapper">
					...
					?>
						<div class="activity-row">
							...
							<div class="col-xs-3 activity-desc">
								<h5><a href="#"><?php echo $row1['cname'];?></a></h5>
								<p><?php echo $row1['jtitle'];?></p>
								<p><?php echo $row1['jlocation'];?></p>
							</div>
							<div class="col-xs-6 activity-desc1"><h5><form action="stujob_c.php" method="POST"><input type="submit" name="job<?php echo $i?>" value="more details"/></h5></form></div>
							<div class="clearfix"> </div>
						</div>
						<?php
				$i=$i+1;}
			...
			?>	
				...
				<div class="activity_box activity_box2">
					...
					$jid=$row2['jid'];
					$_SESSION['job_f'.$i]=$jid;
					$_SESSION['sfid'.$i]=$row2['fid'];
					?>
						<div class="activity-row">
							<div class="col-xs-3 activity-img"><img src='images/1.jpg' class="img-responsive" alt=""/></div>
							<div class="col-xs-3 activity-desc">
								<h5><a href="#"><?php echo $row2['cname'];?></a></h5>
								<p><?php echo $row2['jtitle'];?></p>
								<p><?php echo $row2['jlocation'];?></p>
								<p>From: <?php echo $_SESSION['sfid'.$i];?></p>
							</div>
							<div class="col-xs-6 activity-desc1"><h5><form action="stujob_f.php" method="POST"><input type="submit" name="job<?php echo $i?>" value="more details"/></h5></div>							
							<div class="clearfix"> </div>
						</div>
						<?php
				echo '</form>';
...
<!-- 						<div class="activity-row">
							<div class="col-xs-3 activity-img"><img src='images/1.jpg' class="img-responsive" alt=""/></div>
							<div class="col-xs-3 activity-desc">
								<h5><?php echo $row3['sname']?></h5>
								Email: <?php echo $row3['iid']; $_SESSION['invitation'.$i] = $row3['iid'];?><br>
								University: <?php echo $row3['university']?><br>
							</div>
							<div class="col-xs-6 activity-desc1"><h5><form action="inviteupdate.php" method="POST"><input type = "submit" value = "Accept"  name = "invitation<?php echo $i;?>"><input type = "submit" value = "Refuse"  name = "invitation<?php echo $i;?>"></h5></div>							
							<div class="clearfix"> </div>
						</div> -->
						<form action = "inviteupdate.php" method = "post">	
							<h4><?php echo $row3['sname']?></h4>
							Email: <?php echo $row3['sid']; $_SESSION['invitation'.$i] = $row3['sid'];?><br>
							University: <?php echo $row3['university']?><br>
